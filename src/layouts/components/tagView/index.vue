  ...
  <div class="tagView-wrapper relative white">
    ...
    <div class="scroll-outer" ref="scrollOuter" :style="outerWidth" @mousewheel="handleMouseWheel">
      <div class="scroll-body absolute" ref="scrollBody" :style="{ left: tagBodyLeft + 'px' }">
        <router-link class="tag-item pointer inline-block" :to="item.path" v-for="(item, index) in tagList"
          :key="item.path" :class="isActive(item.path) ? 'activeTag' : ''" v-if="!item.meta.hidden" ref="tagWrapper"
          @contextmenu.prevent="rightEvent(item)">

          <span class="tag-title">{{ item.meta.title }}</span>
          <svg-icon icon="close" v-if="index != 0" class="verticalMiddle tag-icon" :size="14"
            @click.prevent.stop.native="closeTag(item.path)" />
        </router-link>
      </div>
    </div>

    <a-icon type="right" class="absolute pointer tag_btn" style="right:0" @click="handleScroll(-240)"
      v-show="hanldeBtn" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    isActive(path) {
      return path == this.$route.path || this.$route.path == '/view/contract';
    },
    addTag() {
    ...
    closeTag(path) {
      this.$store.dispatch('tagsView/removeTag', path).then(data => {
        data = data.filter((item) => !item.meta?.hidden)
