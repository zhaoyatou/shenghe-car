  ...
  <div id="orderList">
    ...
    <br />
     <a-spin :spinning="spinning">
      <tableList :heights="heights" ref="tableList" :dataList="dataList" :total="total" @business="business" @changePagination="changePagination" :current="pageAge.pageNum">
        <template #opercate>
...
<script>
import serch from '@/views/statistics/businessreport/components/serch';
import tableList from '@/views/statistics/businessreport/components/tableList';
  ...
  methods: {
    ...
    async business(serchValue = {}) {
      try {
        ...
        this.spinning = true
        const result = await business(obj)
        this.spinning = false
        console.log(result)
        this.dataList = Array.from(new Set(result?.data?.list))  || []
        this.total = result?.data?.total || 0
      } catch (error) {
        console.log('加载列表接口(business)的错误信息--->', error)
         this.dataList = []
        this.spinning = false
      }
    },
