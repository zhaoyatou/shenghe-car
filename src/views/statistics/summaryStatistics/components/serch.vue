  ...
  <a-card :loading="loadingCard" size="small" :hoverable="true" :bordered="false">
    <div id="components-form-demo-advanced-search">
      <a-form class="ant-advanced-search-form" :label-col="formItemLayout.labelCol"
        ...
        <a-form-item label="统计范围：" :label-col="formItemLayout.labelCol" :wrapper-col="formItemLayout.wrapperCol">
    ...
    handleSearch(e) {
      ...
      this.form.validateFields((error, values) => {
        ...
        if (values?.purchasetime?.length > 0) {
          values.startTime = moment(values?.purchasetime[0]).format('YYYY-MM')
          values.endTime = moment(values?.purchasetime[1]).format('YYYY-MM')
          delete values?.purchasetime
        }
        console.log('form结果: ', values);
        values.pageNum = 1
        this.$parent.collect(values, null)
      });
    },
